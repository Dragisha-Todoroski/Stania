   ...
   <div className='wrapper'>
    <div className='toprop_content flex flex-col gap-10'>
     ...
     <div className='topprop_list grid lg:grid-cols-4 md:gap-6 xs:grid-cols-2 gap-4 grid-cols-1'>
      {listContent.map(({ id, imgsrc, alt, title, location, grade, reviews }) => (
       <div
        onClick={() => router.push(`/units/${id}`)}
        className='topprop_list_item  rounded-xl overflow-clip h-full drop-shadow-md'
        key={id}
       >
        <img loading='lazy' src={imgsrc} alt={alt} className='projects_element_image w-full' />
        <div className='topprop_item_text xs:px-6 xs:pb-8 xs:pt-4 px-4 pb-6 pt-2 max-h-[200px] bg-[#F6F6F6] flex flex-col gap-3'>
         <h5 className='lg:text-2xl md:text-xl xs:text-lg font-bold text-gray-800'>{title}</h5>
         <p className='lg:text-lg md:text-base text-sm font-semibold text-gray-600'>{location}</p>
         <div className='lg:text-base md:text-sm text-xs flex  gap-3 items-center'>
          <span className='bg-[#3F7D20] text-gray-50 px-2 py-1 rounded'>{grade}</span>
